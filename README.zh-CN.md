...
真实入口：双击 [`启动需求引擎.bat`](启动需求引擎.bat) / [`启动需求引擎.vbs`](启动需求引擎.vbs)，或在 [`req-engine/`](req-engine/) 里执行 `cargo run -- desktop`。
...
`req-engine` 反过来：**本地状态机 + 座位**。智能留在 Agent 宿主（Cursor / Claude / Codex / Grok …）。引擎只存事实、拒绝非法动词、标出谁坐在座位上。
...
桌面「复制讨论侧 / 复制实现侧」会把**匹配码 + 接入 Prompt** 放进剪贴板。贴到 **Agent 宿主** 的 MCP 配置里——看板本身不是 MCP Client。
...
1. 安装 [Rust](https://rustup.rs) 和 [WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)（多数 Win10/11 已带）。
2. 双击 `启动需求引擎.bat`。首次若无二进制会 `cargo build --release`，再由 VBS 以**隐藏控制台**启动 `desktop --home req-engine/data --port 7420`。
3. 在看板建项目，复制讨论或实现接入包，登记到 Cursor / Claude Desktop / Codex / 任意 MCP 宿主：
...
# 原生窗口（自动建空库；注入 admin token）
cargo run -- desktop --home ./data
# 可选演示数据：  --seed-if-missing
...
不传 `--home` / `REQ_ENGINE_HOME` 时，默认是 `%USERPROFILE%\.req-engine`（Unix：`~/.req-engine`）。双击启动器固定 `req-engine/data`，保证窗口和 MCP 落在同一份库。

冒烟（真 HTTP）：`req-engine/scripts/smoke.ps1`。
...
MIT。目标仓库：[github.com/Player-YN/req-engine](https://github.com/Player-YN/req-engine)。
